create database	landingpage_data;
use landingpage_data;
...
zip varchar(255),
countryID int,
foreign key (countryID) references countries(id)  ON DELETE NO ACTION ON UPDATE NO ACTION
);
...
description text,
typeID int,
foreign key (typeID) references type_of_notification(id) ON DELETE NO ACTION ON UPDATE NO ACTION
);
